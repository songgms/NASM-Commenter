...
%define SYS_EXIT 60             ; [nasm-commenter] 定义宏常量 SYS_EXIT = 60

section .data                   ; [nasm-commenter] 声明已初始化数据段 .data
    text db "nasm", 0           ; [nasm-commenter] 定义字符串 text："nasm", 0

section .text                   ; [nasm-commenter] 声明代码段 .text
    global _start               ; [nasm-commenter] 导出符号 _start（链接器可见）

_start:                         ; [nasm-commenter] 函数入口：_start
    mov rdi, text               ; [nasm-commenter] 将标签 text 的地址加载到 rdi
    xor al, al                  ; [nasm-commenter] al 清零（自身异或）
    mov rcx, -1                 ; [nasm-commenter] 将立即数 -1 加载到 rcx
    repne scasb                 ; [nasm-commenter] 重复扫描直到 [rdi] 字节等于 al（找到结尾 0，strlen 的实现）
    mov rax, 1                  ; [nasm-commenter] 系统调用号 1（write）
    mov rdi, 1                  ; [nasm-commenter] 参数1：fd = 1（1 为 stdout）
    mov rsi, text               ; [nasm-commenter] 参数2：输出缓冲区地址
    mov rdx, 4                  ; [nasm-commenter] 参数3：输出字节数
    syscall                     ; [nasm-commenter] 执行 write 系统调用输出字符串
    mov rax, SYS_EXIT           ; [nasm-commenter] 系统调用号 60（exit）
