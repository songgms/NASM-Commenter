...
为 NASM 汇编代码自动生成高质量中文/英文注释的 VSCode 扩展。**默认以虚拟注释（幽灵文字）预览、不修改文档**，确认后再一键写入。离线规则引擎为主，LLM 为可选增强——确定性、隐私友好、无 API Key 也完全可用。
...
> 完整功能介绍见 [docs/features.md](./docs/features.md)。

- **虚拟注释预览（默认）**：注释以幽灵文字在行尾展示，不修改文档；悬停可看指令语义，随时开关
- **智能行内/行上方注释**：确认预览后一键写入文档；基于数据驱动的指令语义模板（151 条常用指令，含 60 条 SIMD，中英双语）
- **上下文追踪**：寄存器常量传播（含 `eax`→`rax` 别名）、栈帧识别、系统调用参数回溯
- **惯用模式识别**：函数序言/结尾、write/exit 系统调用、strlen 扫描、内存块拷贝、cmp+jcc 条件分支、计数循环等 28 个模式优先整段注释
- **ABI 自动检测**：Linux x64 / Linux x86（int 0x80）/ macOS（0x2000000 基址），可手动指定
- **Hover 提示**：指令语义、标志位影响、寄存器惯例、系统调用号 → 调用名与参数、%define 宏 → 定义值
- **自动补全**：命令位置补全指令名（附语义说明），操作数位置补全寄存器与文档标签
- **实时诊断**（10 类）：未知指令、跳转到未定义标签、未引用标签、标签重复定义、栈帧失衡、寄存器无效赋值、预处理嵌套不匹配、内存-内存操作、复杂宏、条件汇编不确定
- **符号导航**：转到定义、查找引用、重命名（标签/常量/宏/结构体字段）、文档大纲
- **基础格式化**：标签/助记符/操作数列对齐（format.enable 开关，不破坏注释）
- **函数块注释**：一键生成 函数名/功能/参数/返回/破坏的寄存器
- **幂等与移除**：重复注释自动跳过；移除命令按规则引擎当前生成结果匹配移除；另有「去掉所有注释」（含手写注释，需确认）
...
| `NASM Commenter: 去掉所有注释` | — | 清除文件内全部注释（含手写，需确认） |
| `NASM Commenter: 切换虚拟注释预览` | — | 开关幽灵文字预览 |
...
|--------|------|------|
| `enable` | `true` | 总开关 |
| `virtual` | `true` | 虚拟注释预览（幽灵文字，不修改文档） |
...
| 虚拟注释点击写入与颜色自定义未实现 | LSP InlayHint 协议不支持；颜色可用 VSCode `editorInlayHint.*` 主题设置 |
| LLM 增强默认 augmentMode=supplement（仅虚拟预览不写入） | 与「先虚拟不直接写」一致；写入需 fallback 模式且不可自动移除 |
| LSP 独立使用通过 npm bin（nasm-commenter-server）而非独立二进制 | 零打包工具链成本，Neovim 等编辑器可直接配置 |
| %include 符号合并仅作用于悬停/补全/大纲；逐行语义替换仍限主文档 | lexer 保持无 fs 纯解析，行号稳定 |
| 知识库预览小工具降级为 scripts/preview-template.js CLI | 无需 Webview 开发即可预览注释效果 |
